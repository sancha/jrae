...
public class RAEPropagation {
	...
	public Tree ForwardPropagate(Theta theta, DoubleMatrix WordsEmbedded,
		...
		for (int j = 0; j < SentenceLength - 1; j++) {
			 ...
			 **/
			DoubleMatrix C1 = WordsEmbedded.getColumns(ArraysHelper.makeArray(
					0, NumComponents - 2));
			DoubleMatrix C2 = WordsEmbedded.getColumns(ArraysHelper.makeArray(
					1, NumComponents - 1));
	...
	public void BackPropagate(Tree tree, Theta theta, int[] WordsIndexed) {
		...
		while (!ToPopulate.empty()) {
			...
			if (!CurrentNode.isLeaf()) {
				...
				GW1.addi((CurrentDelta.mmul(CurrentNode.LeftChild.Features
						.transpose())));
				GW2.addi((CurrentDelta.mmul(CurrentNode.RightChild.Features
						.transpose())));
		...
		for (int l = 0; l < SentenceLength; l++)
			DoubleMatrixFunctions.IncrementColumn(GWe_total, WordsIndexed[l],
					GL.getColumn(l));
	}
	...
	public void BackPropagate(Tree tree, FineTunableTheta theta,
		...
		while (!ToPopulate.empty()) {
			...
			if (!CurrentNode.isLeaf()) {
				...
				GWCat = GWCat
						.addi(CurrentNode.catDelta.mmul(A1Norm.transpose()));
				...
				GWCat = GWCat.addi(CurrentNode.catDelta
						.mmul(CurrentNode.Features.transpose()));
				Gbcat = Gbcat.addi(CurrentNode.catDelta);

				DoubleMatrixFunctions.IncrementColumn(GL, CurrentNode.NodeName,
						(((NodeW.transpose()).mmul(CurrentNode.ParentDelta))
								.addi(theta.Wcat.transpose().mmul(
										CurrentNode.catDelta))).subi(delta));
			}
		}
	...
	/**
	 * Pre-allocating memory for the gradients Line 18-28 in
	...
	private DoubleMatrix UpdateEmbedding(DoubleMatrix CurrentEmbedding,
		...
		int[] rightColumns = ArraysHelper.makeArray(Column + 2,
				CurrentEmbedding.columns - 1);
		...
		if (leftColumns == null && rightColumns == null)
			...
			return DoubleMatrix.concatHorizontally(
					CurrentEmbedding.getColumns(leftColumns), ColumnVector);
		else
			return DoubleMatrix.concatHorizontally(CurrentEmbedding
					.getColumns(leftColumns), DoubleMatrix.concatHorizontally(
